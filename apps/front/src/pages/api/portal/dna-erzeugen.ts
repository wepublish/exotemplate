 ...
 *
 * POST → Logo-Gate ZUERST (Fix-Runde 1, Important): ohne hochgeladenes Logo
 *   (faas_medien.logo_hochgeladen, siehe /api/portal/logo, Pflicht-Erststep)
 *   kein Erzeugungslauf, auch nicht bei direktem POST unter Umgehung des
 *   Client-Guards in dna.tsx. Erst dann startet (oder findet den bereits
 *   laufenden) Job über `starteGenerateDnaJob` (siehe
 *   /api/medium-knowledge/generate-dna, Step 1 der Task-7-Extraktion),
 *   dieselbe Pipeline (sammeln, verdichten, messen, aktivieren), nur dass
 *   das Medium AUSSCHLIESSLICH aus der Portal-Session kommt, nie aus dem Body.
 *   → 202 { job_id, status: 'running' }  frisch gestartet
 *   → 200 { job_id, status: 'running' }  war bereits am Laufen (Dedup)
 ...
 *   → 502 { error }  Directus nicht erreichbar (Logo-Gate-Lookup)
 *
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    return res.status(gestartet.running ? 200 : 202).json({ job_id: gestartet.jobId, status: 'running' })
  }

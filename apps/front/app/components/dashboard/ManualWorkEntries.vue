...
import type { ClientPeriod, ManualWorkEntry } from '~~/types/DirectusTypes'
  ...
  const sum = computed<number>(() => manualWorkingEntriesComp.getSumByClientPeriod((selectedClientPeriod.value?.manualWorkEntries || []) as ManualWorkEntry[]))
  ...
  <div class="flex-1 w-full">  
    <UPageCard>
      <template #default>
        ...
        <UTable
          ref="table"
          :data="((selectedClientPeriod?.manualWorkEntries || []) as ManualWorkEntry[])"
          :columns="columns"
          sticky
        >
          <template #date-cell="{row}">
            {{ new Date(row.original.date as string).toLocaleDateString('de', {dateStyle: 'medium'}) }}
          </template>

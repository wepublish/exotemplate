...
import { createError, InvalidPayloadError, ForbiddenError, ContainsNullValuesError } from '@directus/errors'
...
export default defineEndpoint((router, context) => {
    ...
    try {
      ...
      if (!clientPeriodId) {
        return next(new InvalidPayloadError({reason: 'Missing param clientPeriodId'}))
      }
      ...
      const clientPeriodService = new ItemsService<ClientPeriod>('Clients_Periods', {schema, accountability})

      const clientPeriod = await clientPeriodService.readOne(clientPeriodId)

      if (!clientPeriod) {
        return next(new ContainsNullValuesError({collection: 'Clients_Periods', field: 'id'}))
      }
      ...
      if (!clientId) {
        return next(new ContainsNullValuesError({collection: 'Clients_Periods', field: 'Clients_id'}))
      }

      if (!periodId) {
        return next(new ContainsNullValuesError({collection: 'Clients_Periods', field: 'Periods_id'}))
      }

      const clientService = new ItemsService<Client>('Clients', {schema,accountability})
      const periodService = new ItemsService<Period>('Periods', {schema, accountability})
      ...
      if (!clockCustomerid) {
        return next(new ContainsNullValuesError({collection: 'Client', field: 'clockodo_customer_id'}))
      }
      if (!jiraPrefix) {
        return next(new ContainsNullValuesError({collection: 'Client', field: 'jira_short_code'}))
      }
...
function calculateSums (entryGroupsWithinPeriod: EntryGroups): EntryGroupsWithSums {
  const sums: Sums = {
    billableHours: 0,
    nonBillableHours: 0
  }
  ...
  for (let entryGroup of entryGroupsWithinPeriod.groups) {
    sums.billableHours += entryGroup?.billability?.billableTotal || entryGroup.duration
...
async function groupAndDecorateJiraIssues (
  ...
  await decorateWithPastEntries(jiraGroup, jiraPrefix, env, clockodoCustomerId, timePeriodFrom)
...
function decorateBillability (jiraGroup: EntryGroup): void {
  ...
  for (let issue of jiraGroup.sub_groups) {
    ...
    const durationJira = Number.parseFloat(issue.jiraIssue?.fields.customfield_10028 ?? '0') * 60 * 60
    ...
    if (durationJira > 0) {
      ...
      if (durationCurrent > jiraAvailable) {
        ...
        billablePart = (durationCurrent - minZeroJiraAvailable) * BILLABLE_PART_WEP
      } else {
...
async function decorateWithJiraIssues (jiraGroup: EntryGroup, jiraPrefix: string, env: ValidatedEnv): Promise<void> {
...
async function getEstimatesFromJira (issueKeys: string[], env: ValidatedEnv): Promise<JiraIssue[]> {

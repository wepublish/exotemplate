...
SCOUT_SLACK_CHANNEL = os.environ.get("FAAS_SCOUT_CHANNEL", "C0B7SD7JCEM")  # #faas-admin
...
def _slack_token() -> str | None:
    """xoxb-Bot-Token aus ~/.hermes/config.yaml (gleiches Muster wie faas_briefing_push)."""
    try:
...
def melde_neue_openings(eintraege: list[dict[str, Any]]) -> None:
    """Postet neu gefundene Foerder-Openings zum Review nach Slack (#faas-admin).
    Best-effort: Fehler werden nur geloggt und brechen den Scout-Lauf NICHT ab."""
    if not eintraege:
        return
    token = _slack_token()
    if not token:
        log.warning("Kein Slack-Token in ~/.hermes/config.yaml — Meldung uebersprungen.")
        return
    zeilen = [f"*FaaS-Radar: {len(eintraege)} neue Foerder-Opening(s)* (Review noetig)"]
    for e in eintraege:
        titel = (e.get("titel") or "ohne Titel")[:120]
        kat = e.get("kategorie") or "—"
        dl = f" · Frist {e['deadline']}" if e.get("deadline") else ""
        url = e.get("url") or ""
        zeilen.append(f"• {titel} [{kat}]{dl}{(' · ' + url) if url else ''}")
    zeilen.append("_Review in Directus/App, status = scout_unbestaetigt._")
    try:
        data = json.dumps({"channel": SCOUT_SLACK_CHANNEL, "text": "\n".join(zeilen)}).encode()
        req = urllib.request.Request(
        ...
        if r.get("ok"):
            log.info("Slack-Meldung gepostet (%d Openings, Channel %s).", len(eintraege), SCOUT_SLACK_CHANNEL)
        else:
            log.warning("Slack-Meldung abgelehnt: %s", r.get("error"))
    except Exception as e:  # noqa: BLE001 - Slack darf den Lauf nie kippen
        log.warning("Slack-Meldung fehlgeschlagen: %s", e)
...
def main() -> None:
        ...
        for kandidat in kandidaten:
            ...
            if neue_id:
                ...
                neue_eintraege.append({
                    ...
                    "kategorie": kandidat.get("kategorie"),
                    "deadline": kandidat.get("deadline"),
    ...
    # Neue Openings zum Review nach Slack melden (Michis Wunsch; best-effort).
    melde_neue_openings(neue_eintraege)
